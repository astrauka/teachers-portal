...
describe('makeTeachersProfileViews', () => {
    ...
    const teachersProfileView1 = {
        ...teachersProfile1,
        country: country1.title,
        language: language1.title,
    };
    const teachersProfileView2 = {
        ...teachersProfile2,
        country: country2.title,
        language: language2.title,
    };
    const teachersProfileViews = [teachersProfileView1, teachersProfileView2];
    const getCountryRepository = (countries) => createStubInstance(CountryRepository, (stub) => {
        stub.fetchCountriesByIds.resolves(countries);
    });
    const getLanguageRepository = (languages) => createStubInstance(LanguageRepository, (stub) => {
        stub.fetchLanguagesByIds.resolves(languages);
    });
    const buildTestContext = ({ countryRepository = getCountryRepository(countries), languageRepository = getLanguageRepository(languages), } = {}) => ({
        countryRepository,
        languageRepository,
        makeTeachersProfileViews: makeTeachersProfileViewsFactory(countryRepository, languageRepository),
    });
