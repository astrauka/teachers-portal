...
describe('authenticateTeacher', () => {
    ...
    const getGoogleAuthService = () => createStubInstance(GoogleAuthService, (stub) => {
    ...
    const getTeachersService = (teachersInfo) => createStubInstance(TeachersInfoRepository, (stub) => {
        stub.fetchTeacherByEmail.resolves(teachersInfo);
    });
    const getGeneratePassword = (password) => stubFn().resolves(password);
    const buildTestContext = ({ googleAuthService = getGoogleAuthService(), usersService = getUsersService(), teachersInfoRepository = getTeachersService(teachersInfo), generatePassword = getGeneratePassword(password), } = {}) => ({
        googleAuthService,
        usersService,
        teachersInfoRepository,
        generatePassword,
        authenticateTeacher: authenticateTeacherFactory(googleAuthService, teachersInfoRepository, usersService, generatePassword),
    });
    it('should register teacher, update teacher info and return member', async () => {
        const { googleAuthService, usersService, teachersInfoRepository, generatePassword, authenticateTeacher, } = buildTestContext();
