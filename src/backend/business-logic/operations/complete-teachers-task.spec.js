...
describe('completeTeachersTask', () => {
    ...
    const getTeachersInfoRepository = (completedTasks) => createStubInstance(TeachersInfoRepository, (stub) => {
        stub.completeTask.resolves();
        stub.fetchCompletedTasks.resolves(completedTasks);
    });
    const getTaskRepository = (task) => createStubInstance(TaskRepository, (stub) => {
        stub.fetchTaskByNumberOrThrow.resolves(task);
    });
    const buildTestContext = ({ getCurrentTeachersInfo = getGetCurrentTeachersInfo(teachersInfo), teachersInfoRepository = getTeachersInfoRepository(completedTasks), taskRepository = getTaskRepository(task), } = {}) => ({
        getCurrentTeachersInfo,
        teachersInfoRepository,
        taskRepository,
        completeTeachersTask: completeTeachersTaskFactory(getCurrentTeachersInfo, teachersInfoRepository, taskRepository),
    });
    it('should mark task as done', async () => {
        const { getCurrentTeachersInfo, teachersInfoRepository, taskRepository, completeTeachersTask, } = buildTestContext();
    ...
    context('on task completed', () => {
        ...
        it('should do nothing', async () => {
            const { getCurrentTeachersInfo, teachersInfoRepository, taskRepository, completeTeachersTask, } = buildTestContext({ teachersInfoRepository: getTeachersInfoRepository(completedTasks) });
            await completeTeachersTask(task.number);
            expect(getCurrentTeachersInfo).calledOnceWithExactly();
            expect(taskRepository.fetchTaskByNumberOrThrow).calledOnceWithExactly(task.number);
