...
describe('getCurrentTeachersTasks', () => {
    ...
    const getTaskRepository = (tasks) => createStubInstance(TaskRepository, (stub) => {
    ...
    const buildTestContext = ({ taskRepository = getTaskRepository(tasks), teachersInfoRepository = getTeachersInfoRepository([completedTask]), getCurrentTeachersInfo = getGetCurrentTeachersInfo(teachersInfo), } = {}) => ({
        taskRepository,
        teachersInfoRepository,
        getCurrentTeachersInfo,
        getCurrentTeachersTasks: getCurrentTeachersTasksFactory(taskRepository, teachersInfoRepository, getCurrentTeachersInfo),
    });
    it('should return current teacher profile', async () => {
        const { taskRepository, teachersInfoRepository, getCurrentTeachersInfo, getCurrentTeachersTasks, } = buildTestContext();
