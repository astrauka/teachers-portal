...
async function configureBackend() {
  ...
  await copy(BACKEND_DIST_DIR, BACKEND_SRC_DIR, {filter: '**/*.js'});
...
async function configureFrontendPublic() {
  ...
  await copy(PUBLIC_DIST_DIR, PUBLIC_SRC_DIR, {filter: '**/*.js'});
...
async function configureFrontendPages() {
  ...
  const typescriptPages = fs.readdirSync(PAGES_DIST_DIR).map(file => file.split('.js')[0]);
