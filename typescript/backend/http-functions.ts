...
import { WixHttpFunctionRequest, WixHttpFunctionResponse } from './types/wix-types';
import { withLogger } from './utils/logger';
import { ensureTestUserTokenCorrect, getResponse, withRouteErrorHandler } from './utils/request';

export function post_teachers_invite(
...
export function post_teachers_cleanFields(
  request: WixHttpFunctionRequest
): Promise<WixHttpFunctionResponse> {
  return withRouteErrorHandler(async () => {
    await ensureTestUserTokenCorrect(request);
